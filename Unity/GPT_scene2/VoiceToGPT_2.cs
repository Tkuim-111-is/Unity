...
//wifi
public class VoiceToGPT_2 : MonoBehaviour
{
    public Transform player;
    public OVRRayHelper rayHelper;
    public LayerMask objectLayer;
    public OpenAITTS_2 tts;
    ...
    void Start()
    {
        ...
        tts = FindObjectOfType<OpenAITTS_2>();
    ...
    // NPC1詢問是否可以進去，玩家回應可以的時候該做的事情
    private IEnumerator npc1_is_not_clean()
    {
    ...
    {
        string APIPath = Path.Combine(Application.dataPath, "Unity", "GPT_scene2", "api_key.json");
                     ...
                     if (!string.IsNullOrEmpty(transcribedText))
                     {
                        ...
                        GenerateResponse(transcribedText, status_prompt, async (gptResponse) =>
                        {
                            ...
                            else if (gptResponse.Contains("第一階段失敗"))
                            {
                                ...
                                _ = sub_and_tts("裡面還有許多資料沒收欸，我們進去不好吧");
                                StartCoroutine(npc1_is_not_clean());
                            }
                            else
                            {
